    ...
    <div className="space-y-4 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-gray-500 text-xs text-center md:text-left">Kampanya gönderimleri için hedef kitle listeleri.</p>
        </div>
      ...
      {viewMode === 'board' && (
        ...
        >
            ...
            <div key={group.id} className="flex-none w-72 bg-gray-50/50 border border-border rounded-xl flex flex-col shadow-sm">
              <div className="p-3 border-b border-border bg-white rounded-t-xl flex items-center justify-between">
                <div className="flex items-center gap-2 overflow-hidden">
                  <div className="bg-orange-100 p-1.5 rounded-lg text-orange-600 flex-none">
                    <Layers size={16} />
                  </div>
          ...
          >
            ...
            <span className="font-bold text-[11px] uppercase tracking-wider">Yeni Kampanya Grubu Ekle</span>
          </button>
        </div>
      )}
                    ...
                    <td className="px-6 py-4 sticky left-0 bg-white border-r border-gray-100 group-hover:bg-gray-50">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex flex-col">
                          <span className="font-bold text-gray-800 leading-none mb-0.5">{contact.fullName}</span>
                          <span className="text-[10px] text-gray-400">{contact.primaryPhone}</span>
                        </div>
                      </div>
                    </td>
                    {groups.map((group: Group) => {
                ...
                return (
                  <div key={contact.id} className={`flex items-center justify-between p-2.5 rounded-lg border transition-all ${isMember ? 'bg-orange-600 text-white border-orange-600' : 'bg-white border-border text-gray-900 hover:border-blue-300'}`}>
                    <div className="flex items-center gap-2.5 min-w-0 flex-1">
                      ...
                      <div className="flex flex-col min-w-0">
                        <span className="font-bold text-xs tracking-tight truncate">{contact.fullName}</span>
                        <span className={`text-[10px] truncate ${isMember ? 'text-white/60' : 'text-gray-400'}`}>{contact.primaryPhone}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button 
