...
export async function POST() {
  ...
  try {
    // SADECE 'ilmihal' OTOMASYONLARINI ÇEK
    const { data: auto, error: autoError } = await supabase
      .from('content_automation')
      .select('*')
      .eq('is_active', true)
      .eq('content_type', 'ilmihal')
      .maybeSingle();

    if (autoError || !auto) return NextResponse.json({ success: false, message: 'Aktif İlmihal otomasyonu bulunamadı.' });

    // SIRADAKİ İLMİHALİ BUL
    ...
    if (itemError || !nextItem) return NextResponse.json({ success: false, message: 'Gönderilecek yeni İlmihal bilgisi bulunamadı.' });
